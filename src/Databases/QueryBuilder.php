<?php
  ...
  class QueryBuilder{
    ...
    function exec($array=null):self{
      $sql=join($this->query); //convertimos en string el array de strings
      $this->stmt=$this->query($sql); //es $statement de la function query
      $this->stmt->execute(); //devuelce objeto con execute
      return $this; //El resultado no sale hasta que hagas fetch
    }
    
    function fetch():?array{
      $rows=$this->stmt->fetchAll(\PDO::FETCH_OBJ);
      if($rows){
        return $rows;//si tengo valores y devuelvo filas
      }
      return null;
    }
  ...
  */
  ...
  /*
  un string cada metodo
  llamas a si mismo el objeto y modificas lo que quieres. No asobreescribes
  */ 